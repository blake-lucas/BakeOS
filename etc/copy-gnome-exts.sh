...
for user in /var/home/*; do mkdir $user/.config/autostart && mkdir $user/.local/share/gnome-shell/extensions; done
...
for user in /var/home/*; do chown $(basename $user):$(basename $user) $user/.config/autostart $user/.local/share/gnome-shell/extensions -R; done